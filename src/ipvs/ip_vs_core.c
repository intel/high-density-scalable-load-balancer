...
void dp_vs_flow_udp_in(struct rte_mbuf *mbuf)
{
    ...
    if (__dp_vs_in(NULL, mbuf, NULL, AF_INET) != INET_STOLEN) {
        rte_pktmbuf_free(mbuf);
    }
}
...
int dp_vs_init(void)
{
    ...
    err = lb_core_init();
    if (err != EDPVS_OK) {
        RTE_LOG(ERR, IPVS, "fail to init lb %s\n", dpvs_strerror(err));
        goto err_lb;
    }
...
err_laddr:
    dp_vs_proto_term();
err_lb:
    inet_unregister_hooks(dp_vs_ops, NELEMS(dp_vs_ops));
    return err;
}
