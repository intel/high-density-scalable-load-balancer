...
#define this_conn_cache             (dp_vs_conn_cache[rte_lcore_id()])
#define this_conn_fstime            (RTE_PER_LCORE(dp_vs_conn_fstime))
...
static RTE_DEFINE_PER_LCORE(uint64_t , dp_vs_conn_fstime);
...
int dp_vs_get_conn_init_timeout(void)
{
    return conn_init_timeout;
}

/* Set the timestamp. The connection needs to update the fast path cache of ARP or route. */
void dp_vs_set_conn_fstime(uint64_t fstime)
{
    this_conn_fstime = fstime;
}

uint64_t dp_vs_get_conn_fstime(void)
{
    return this_conn_fstime;
}
 ...
 */
static struct rte_mempool *dp_vs_conn_cache[RTE_MAX_LCORE];
                                  ...
                                  struct dp_vs_dest *dest, uint32_t flags)
{
    ...
    new->fs_time = DPVS_CONN_FS_MAGIC;
...
static int conn_init_lcore(void *arg)
{
    ...
    /* connection cache on each core according to NUMA socket */
    netif_get_slave_lcores(&nlcore, &lcore_mask);
    pool_size = conn_pool_size / nlcore;
    snprintf(poolname, sizeof(poolname), "dp_vs_conn_%d", cid);

    dp_vs_conn_cache[cid] = rte_mempool_create(poolname,
                                pool_size,
    ...
    if (!dp_vs_conn_cache[cid]) {
        RTE_LOG(ERR, IPVS, "%s: lcore %d: create conn mempool %s.\n",
                    __func__, cid);
        goto err;
    }
...
again:
    ...
    for ( ; cid < DPVS_MAX_LCORE; cid++)
        if (cid >= 64 || (g_slave_lcore_mask & (1UL << cid)))
            break;
    if (cid >= DPVS_MAX_LCORE || cid >= 64) {
        conn_arr->nconns = got;
        conn_arr->resl = GET_IPVS_CONN_RESL_OK;
        conn_arr->curcid = 0;
        return EDPVS_OK;
