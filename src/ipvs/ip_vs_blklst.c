                                     ...
                                     const union inet_addr *blklst)
{
    ...
    return rte_jhash_2words((uint32_t)vaddr->in.s_addr, ((uint32_t)blklst->in.s_addr),
                dp_vs_blklst_rnd) & DPVS_BLKLST_TAB_MASK;
}

struct list_head *lb_get_blklst_table_head(uint8_t proto, union inet_addr *vaddr,
                                         uint16_t vport, union inet_addr *blklst)
{
    unsigned hashkey;

    hashkey = blklst_hashkey(vaddr, blklst);

    return &this_blklst_tab[hashkey];
}

struct blklst_entry *lb_blklst_lookup(struct list_head *table_head, uint8_t proto, union inet_addr *vaddr,
                                         uint16_t vport, union inet_addr *blklst)
{
    struct blklst_entry *blklst_node;

    if (likely(list_empty(table_head)))
        return NULL;

    list_for_each_entry(blklst_node, table_head, list){
