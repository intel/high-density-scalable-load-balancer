...
static int udp_fast_fnat_in_handler(struct dp_vs_proto *proto,
                    struct dp_vs_conn *conn,
                    struct rte_mbuf *mbuf)
{
    ...
    if (unlikely(!(conn->flags & DPVS_CONN_F_CSUM_UDPOL_IN)))
...
struct dp_vs_proto dp_vs_proto_udp = {
    ...
    .fnat_fast_in_handler = udp_fast_fnat_in_handler,
...
int udp_get_timeout(void)
{
    return udp_timeouts[DPVS_UDP_S_NORMAL];
}

int udp_get_uoa_mode(void)
{
    return g_uoa_mode;
}

int udp_get_max_trail(void)
{
    return g_uoa_max_trail;
}
