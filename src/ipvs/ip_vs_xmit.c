...
static int __dp_vs_fast_xmit_fnat6_v2(struct dp_vs_proto *proto,
                                 struct dp_vs_conn *conn,
                                 struct rte_mbuf *mbuf)
{
    return 0;
...
static int __dp_vs_fast_xmit_fnat4_v2(struct dp_vs_proto *proto,
                                  struct dp_vs_conn *conn,
                                  struct rte_mbuf *mbuf)
{
   struct rte_ipv4_hdr *ip4h = ip4_hdr(mbuf);
   ...
   if (conn->fs_time != dp_vs_get_conn_fstime()) {
        // cache out info
        ...
        //fl4.fl4_tos = ip4h->type_of_service;
        ...
        if (conn->out_dev->flag & NETIF_PORT_FLAG_TX_IP_CSUM_OFFLOAD)
            conn->flags |= DPVS_CONN_F_CSUM_IPOL_OUT;
        if (conn->out_dev->flag & NETIF_PORT_FLAG_TX_TCP_CSUM_OFFLOAD)
            conn->flags |= DPVS_CONN_F_CSUM_TCPOL_OUT;
        if (conn->out_dev->flag & NETIF_PORT_FLAG_TX_UDP_CSUM_OFFLOAD)
            conn->flags |= DPVS_CONN_F_CSUM_UDPOL_OUT;
        ...
        */
        ...
        if (conn->in_dev->flag & NETIF_PORT_FLAG_TX_IP_CSUM_OFFLOAD)
            conn->flags |= DPVS_CONN_F_CSUM_IPOL_IN;
        if (conn->out_dev->flag & NETIF_PORT_FLAG_TX_TCP_CSUM_OFFLOAD)
            conn->flags |= DPVS_CONN_F_CSUM_TCPOL_IN;
        if (conn->out_dev->flag & NETIF_PORT_FLAG_TX_UDP_CSUM_OFFLOAD)
            conn->flags |= DPVS_CONN_F_CSUM_UDPOL_IN;

        hashkey = neigh_hashkey(AF_INET, (union inet_addr *)&conn->in_nexthop.in, conn->in_dev);
        neighbour = neigh_lookup_entry(AF_INET, (union inet_addr *)&conn->in_nexthop.in, conn->in_dev, hashkey);
        if (neighbour && ((neighbour->state == DPVS_NUD_S_REACHABLE) ||
                 (neighbour->state == DPVS_NUD_S_PROBE) ||
                 (neighbour->state == DPVS_NUD_S_DELAY))) { // zhutao fix me, arp state not understanding
            rte_ether_addr_copy(&neighbour->eth_addr, &conn->in_dmac);
            rte_ether_addr_copy(&conn->in_dev->addr, &conn->in_smac);
        } else
            goto errout;

        conn->fs_time = dp_vs_get_conn_fstime();
   }
   ...
   if (unlikely(rte_is_zero_ether_addr(&conn->in_dmac) ||
    ...
    if(proto->fnat_fast_in_handler) {
       err = proto->fnat_fast_in_handler(proto, conn, mbuf);
       if(err != EDPVS_OK)
           return err;
    }
    ...
    ip4h->src_addr = conn->laddr.in.s_addr;
    ip4h->dst_addr = conn->daddr.in.s_addr;
    if (conn->flags & DPVS_CONN_F_CSUM_IPOL_IN) {
        mbuf->ol_flags |= PKT_TX_IP_CKSUM | PKT_TX_IPV4;
    } else {
        ip4h->hdr_checksum = rte_ipv4_phdr_cksum(ip4h, mbuf->ol_flags);
    }
...
static int dp_vs_fast_xmit_fnat_v2(int af,
                              struct dp_vs_proto *proto,
                              struct dp_vs_conn *conn,
                              struct rte_mbuf *mbuf)
{
  return af == AF_INET ? __dp_vs_fast_xmit_fnat4_v2(proto, conn, mbuf)
      : __dp_vs_fast_xmit_fnat6_v2(proto, conn, mbuf);
}
                              ...
                              struct rte_mbuf *mbuf)
{
    ...
    if (!fast_xmit_close && !(conn->flags & DPVS_CONN_F_NOFASTXMIT)) {
        if (!dp_vs_fast_xmit_fnat_v2(AF_INET, proto, conn, mbuf))
            return EDPVS_OK;
#if 0
        dp_vs_save_xmit_info(mbuf, proto, conn);
