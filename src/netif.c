 ...
 *
 * Changes:
 *    Zhu Tao: Adapt to dpdk1911 and support i40e and ice NIC.
 */
#include <unistd.h>
#include <fcntl.h>
#include <assert.h>
...
static netif_flow_handler_t g_flow_handler;

/* max threshold of transmitted packets diff between this loop and previous loop */
#ifndef TX_IDLE_THRESHOLD_MAX
#define TX_IDLE_THRESHOLD_MAX 1
#endif 
/* total packets transmitted by one core from startup time to previous loop */
static RTE_DEFINE_PER_LCORE(uint64_t, this_lcore_prev_opackets);
/*
 * Threshold of transmitted packets diff between this loop and previous loop.
 * It can be updated via API. If below this threshold, the core is regarded as
 * idle.
 */
static RTE_DEFINE_PER_LCORE(uint32_t, this_lcore_tx_idle_threshold) = TX_IDLE_THRESHOLD_MAX;
static RTE_DEFINE_PER_LCORE(uint32_t, this_lcore_tx_idle_credit)    = 0;
...
int netif_set_flow_handler(netif_flow_handler_t cb)
{
    if (g_flow_handler)
        return EDPVS_EXIST;
...
int netif_clear_flow_handler(netif_flow_handler_t cb)
{
    if (g_flow_handler != cb)
        return EDPVS_EXIST;
...
{
    struct netif_lcore_loop_job *cur;
    if (unlikely(NULL == lcore_job || lcore_job->type >= NETIF_LCORE_JOB_TYPE_MAX))
        return EDPVS_INVAL;
...
/* Note: Lockless, lcore_conf is set on initialization stage by cfgfile /etc/hdslb.conf.
config sample:
static struct netif_lcore_conf lcore_conf[DPVS_MAX_LCORE + 1] = {
    {.id = 1, .nports = 2, .pqs = {
    ...
    while (cpu_left > 0) {
        ...
        list_for_each_entry_reverse(queue, &worker_min->port_list, queue_list_node) {
            ...
            if (port)
                ...
                rte_panic("Can't find port by name: %s\n", queue->port_name);
                pid = NETIF_PORT_ID_INVALID;
            }
            lcore_conf[id].pqs[tk].id = pid;
            lcore_conf[id].pqs[tk].nic_type = port->nic_type;
...
int netif_show_stats(uint8_t cid)
{
    int i;
    uint64_t ipackets = 0, impackets = 0, opackets = 0;

    if (cid) {
        ipackets += lcore_stats[cid].ipackets;
        impackets += lcore_stats[cid].impackets;
        opackets += lcore_stats[cid].opackets;
    } else {
        for (i=0; i<DPVS_MAX_LCORE; i++) {
            ipackets += lcore_stats[i].ipackets;
            impackets += lcore_stats[i].impackets;
            opackets += lcore_stats[i].opackets;
        }
    }

    RTE_LOG(WARNING, NETIF, "%s [%d](0: all): %lu inpkts, %lu inmpkts, %lu outpkts\n", __func__, cid, ipackets, impackets, opackets);
...
extern uint16_t rte_pmd_ice_recv_pkts_bulk_alloc(uint16_t port_id, uint16_t queue_id,
			  struct rte_mbuf **rx_pkts, uint16_t nb_pkts, uint8_t *ofb_id, uint8_t *ofb_data);
static inline uint16_t __netif_rx_burst(portid_t pid, struct netif_queue_conf *qconf, uint32_t nic_type)
{
    int nrx = 0, i;

    if (nic_type == NIC_DRV_I40E) {
        return rte_pmd_i40e_recv_pkts_bulk_alloc(pid, qconf->id, qconf->mbufs, NETIF_MAX_PKT_BURST, &qconf->first_flowid, qconf->ofb_data);
    } else if (nic_type == NIC_DRV_ICE) {
        return rte_pmd_ice_recv_pkts_bulk_alloc(pid, qconf->id, qconf->mbufs, NETIF_MAX_PKT_BURST, &qconf->first_flowid, qconf->ofb_data);
    } else {
        nrx = rte_eth_rx_burst(pid, qconf->id, qconf->mbufs, NETIF_MAX_PKT_BURST);
        if (unlikely(nic_type == NIC_DRV_BOND)) {
            for (i=0; i<nrx; i++) {
                if ((qconf->mbufs[i]->ol_flags & PKT_RX_FDIR) && qconf->mbufs[i]->hash.fdir.hi) {
                    if (unlikely(qconf->first_flowid == NETIF_FLOW_INVALID))
                        qconf->first_flowid = (uint8_t)(qconf->mbufs[i]->hash.fdir.hi);

                    qconf->ofb_data[i] = (uint8_t)(qconf->mbufs[i]->hash.fdir.hi);
                }
            }
        }
...
#if 0
static void dump_rss(struct rte_mbuf **mbufs, int nrx)
{
    int i;
    struct rte_mbuf *mbuf;
    ...
    for (i=0; i<nrx; i++) {
        mbuf = mbufs[i];
        printf("... core%d, rss: 0x%x\n", rte_lcore_id(), mbuf->hash.rss);
    }
}
#endif

static inline uint16_t netif_rx_burst(portid_t pid, struct netif_queue_conf *qconf, uint32_t nic_type)
{
    struct rte_mbuf *mbuf;
    int nrx = 0;
    ...
    if (qconf->isol_rxq) {
        ...
        qconf->first_flowid = NETIF_FLOW_INVALID;
        memset(qconf->ofb_data, NETIF_FLOW_GENERAL, NETIF_MAX_PKT_BURST);
        nrx = __netif_rx_burst(pid, qconf, nic_type);
        //dump_rss(qconf->mbufs, nrx);
    }
...
static inline void netif_tx_burst(lcoreid_t cid, portid_t pid, queueid_t qindex)
{
    ...
    nprep = rte_eth_tx_prepare(pid, txq->id, txq->mbufs, txq->len);
    if (unlikely(nprep != txq->len)) {
        RTE_LOG(DEBUG, NETIF, "%s: Burst preparing tx packet failed.\n", __func__);
    }
...
int netif_hard_xmit(struct rte_mbuf *mbuf, struct netif_port *dev)
{
    ...
    pid = dev->id;
#if 0 // Damaging performance, use first txq
...
#endif
    txq = &lcore_conf[lcore2index[cid]].pqs[port2index[cid][pid]].txqs[0];

    if (unlikely(txq->len == NETIF_MAX_PKT_BURST)) {
        netif_tx_burst(cid, pid, 0);
        txq->len = 0;
    }
...
lcore_marked_prefetch_mbuf(struct rte_mbuf **mbufs, uint8_t *ofb_data, uint32_t start, 
                                    uint32_t count, uint8_t markid, uint8_t *nextid)
{
    uint32_t i;

    for (i=start; i<count; i++) {
        if (likely(ofb_data[i] != NETIF_FLOW_GENERAL && ofb_data[i] != NETIF_FLOW_INVALID)) {
            if (likely(ofb_data[i] == markid)) {
...
static inline void
lcore_replace_general_mbuf(struct netif_queue_conf *qconf, uint8_t processed)
{
    uint32_t i, count, index;
    struct rte_mbuf **mbufs = qconf->mbufs;
    uint8_t *ofb_data = qconf->ofb_data;

    count = qconf->len - processed;

    for (i=0,index=0; i<qconf->len || index==count; i++) {
        if (ofb_data[i] == NETIF_FLOW_GENERAL) {
            mbufs[index++] = mbufs[i];
        }
    }
...
static inline uint32_t
lcore_same_marked_flow(struct rte_mbuf **mbufs, uint8_t *ofb_data,
                                    uint32_t count, uint8_t markid, uint8_t *nextid)
{
    uint32_t index, fetchnext, processed = 0;

    index = lcore_marked_prefetch_mbuf(mbufs, ofb_data, 0, count, markid, nextid);
    while (index < count) {
        // fetch next packets;
        fetchnext = lcore_marked_prefetch_mbuf(mbufs, ofb_data, index+1, count, markid, nextid);

        // process index packets
        g_flow_handler(mbufs[index], markid);
        processed++;

        ofb_data[index] = NETIF_FLOW_INVALID;
        index = fetchnext;
    }
    g_flow_handler(NULL, markid);

    return processed;
}

static void lcore_process_marked_flow(struct netif_queue_conf *qconf)
{
    uint8_t nowid = qconf->first_flowid;
    uint8_t nextid = 0; // next markid
    uint8_t processed = 0;

    if (qconf->first_flowid == NETIF_FLOW_INVALID) {
        qconf->marked_cnt = 0;
        return;
    }

    processed = lcore_same_marked_flow(qconf->mbufs, qconf->ofb_data, qconf->len, nowid, &nextid);
    while (nextid) {
        nowid = nextid;
        nextid = 0;
        processed += lcore_same_marked_flow(qconf->mbufs, qconf->ofb_data, qconf->len, nowid, &nextid);
    ...
    qconf->marked_cnt = processed;
    lcore_replace_general_mbuf(qconf, processed);
}

void insert_empty_pkt(void)
{
    g_flow_handler(NULL, 0);
}

void lcore_process_redirect_marked_flow(struct rte_mbuf **mbufs, lcoreid_t cid, uint16_t count)
{
    int i, t;
    ...
    for (i = 0; i < count; i++) {
        ...
        g_flow_handler(mbufs[i], mbufs[i]->hash.fdir.hi);
    }
}
...
static inline int
lcore_update_rx_principal_status(uint16_t rx_len, enum netif_principal_status stat)
{
    enum netif_principal_status curr;

    if (stat == NETIF_PRINCIPAL_STAT_FULL)
        return stat;

    if (rx_len == NETIF_MAX_PKT_BURST) {
        curr = NETIF_PRINCIPAL_STAT_FULL;
    } else if (rx_len >= (NETIF_MAX_PKT_BURST>>1)) {
        curr = NETIF_PRINCIPAL_STAT_HIGH;
    } else if (rx_len == 0) {
        curr = NETIF_PRINCIPAL_STAT_IDLE;
    } else {
        curr = NETIF_PRINCIPAL_STAT_LOW;
    }

    return curr > stat ? curr : stat;
}
...
static int lcore_job_xmit(void *args __rte_unused, int high_stat __rte_unused)
{
    int i, j;
    lcoreid_t cid;
    portid_t pid;
...
static int lcore_job_timer_manage(void *args __rte_unused, int high_stat)
{
    //static uint64_t tm_manager_time[DPVS_MAX_LCORE] = { 0 };
    uint64_t now = rte_get_timer_cycles();
    //portid_t cid = rte_lcore_id();

    lb_lcore_timer_manage(now, high_stat);
#if 0
...
{
/* TODO: FIX ME, if you want to use bond devices */
    return 0;
# if 0
...
static int dpdk_set_mc_list(struct netif_port *dev)
{
    ...
    if ((dev->nic_type == NIC_DRV_I40E) || (dev->nic_type == NIC_DRV_ICE)) {
        return EDPVS_OK;
    }
...
{
    /* TODO: FIX ME, if you want to use filter supported api */
...
{
    /* TODO: FIX ME, if you want to use filter ctrl api */
#if 0
...
static int dpdk_set_fdir_filt(struct netif_port *dev, enum rte_filter_op op,
                              const struct rte_eth_fdir_filter *filt)
{
    /* TODO: FIX ME, if you want to use  fdir filter*/
#if 0
...
/* return > 0: flow id, < 0: error */
static int dpdk_flow_create(struct netif_port *dev,
                           ...
                           const struct rte_flow_action *actions)
{
    ...
    struct netif_flow_rule *rule;

    memset(&error, 0x22, sizeof(error));
    flow = rte_flow_create(dev->id, attr, patterns, actions, &error);
    if (!flow) {
        RTE_LOG(ERR, NETIF, "%s: fail to create dip2queue flow on port %d: %s\n",
                __func__, dev->id, error.message);
        return -1;
    }

    dev->flow_next++;
    if (dev->flow_next == NETIF_FLOW_MAX) {
        dev->flow_next--;
        RTE_LOG(ERR, NETIF, "Flow was overflow in %d!", dev->id);
        goto err;
    }

    rule = rte_zmalloc("port", sizeof(struct netif_flow_rule), RTE_CACHE_LINE_SIZE);
    if (!rule) {
        RTE_LOG(ERR, NETIF, "Alloc malloc fail!\n");
        goto err;
    }

    rule->flow_id = dev->flow_next;
    rule->flow = flow;
    list_add_tail(&rule->list, &dev->flow);

    return rule->flow_id;

err:
    rte_flow_destroy(dev->id, flow, &error);
    return -1;
}

static int dpdk_flow_destroy(struct netif_port *dev, int flow_id)
{
    struct netif_flow_rule *rule, *n;
    struct rte_flow_error error;
    int err;

    list_for_each_entry_safe(rule, n, &dev->flow, list) {
        if (rule->flow_id == flow_id) {
            err = rte_flow_destroy(dev->id, rule->flow, &error);
            if (err) {
                RTE_LOG(ERR, NETIF, "Port %d destroy flow id %d fail: %s!\n", dev->id, flow_id, error.message);
                return err;
            }
            list_del(&rule->list);
            rte_free(rule);
    ...
    RTE_LOG(ERR, NETIF, "Can't find flow id: %d on port %d!\n", flow_id, dev->id);
    return -1;
}

static rte_spinlock_t g_flow_lock;
static struct netif_ops dpdk_netif_ops = {
    ...
    .op_flow_create = dpdk_flow_create,
    .op_flow_destroy = dpdk_flow_destroy,
};
...
int netif_flow_mark(int af, uint8_t proto, uint16_t port, uint32_t markid,
                    union inet_addr *addr, struct netif_port *dev)
{
    int flow_id;
    ...
    struct rte_flow_item_tcp tcp, tcp_mask;
    ...
    if (af == AF_INET) {
        ...
        ipv4.hdr.dst_addr = addr->in.s_addr;
        //ipv4.hdr.dst_addr = rte_cpu_to_be_32(0xAFFFFFE); //10.255.255.254
        ipv4_mask.hdr.dst_addr = 0xFFFFFFFF; // 255.255.255.255
        ...
        rte_memcpy(ipv6.hdr.dst_addr, &addr->in6, sizeof(struct in6_addr));
        rte_memcpy(ipv6_mask.hdr.dst_addr, "\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff", sizeof(struct in6_addr));
    ...
    flow_id = dpdk_flow_create(dev, &attr, patterns, actions);
    if (flow_id < 0) {
        RTE_LOG(ERR, NETIF, "%s: fail to create mark flow on port %d\n",
                __func__, dev->id);
        return EDPVS_INVAL;
    }

    RTE_LOG(INFO, NETIF, "Create mark rule success on port %d\n", dev->id);
    return flow_id;
}

int netif_flow_src_mark(int af, uint8_t proto, uint16_t port, uint32_t markid,
                    union inet_addr *addr, struct netif_port *dev)
{
    int flow_id;
    struct rte_flow_attr attr;
    ...
    if (af == AF_INET) {
        memset(&ipv4, 0, sizeof(ipv4));
        memset(&ipv4_mask, 0, sizeof(ipv4_mask));
        ipv4.hdr.src_addr = addr->in.s_addr;
        //ipv4.hdr.dst_addr = rte_cpu_to_be_32(0xAFFFFFE); //10.255.255.254
        ipv4_mask.hdr.src_addr = 0xFFFFFFFF; // 255.255.255.255
        patterns[1].type = RTE_FLOW_ITEM_TYPE_IPV4;
        patterns[1].spec = &ipv4;
        patterns[1].mask = &ipv4_mask;
    } else {
        memset(&ipv6, 0, sizeof(ipv6));
        memset(&ipv6_mask, 0, sizeof(ipv6_mask));
        rte_memcpy(ipv6.hdr.src_addr, &addr->in6, sizeof(struct in6_addr));
        rte_memcpy(ipv6_mask.hdr.src_addr, "\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff", sizeof(struct in6_addr));
        patterns[1].type = RTE_FLOW_ITEM_TYPE_IPV6;
        patterns[1].spec = &ipv6;
        patterns[1].mask = &ipv6_mask;
    }
...
static int inline
netif_i40e_set_nat_rss_hash(struct netif_port *indev, struct netif_port *outdev)
{
    struct rte_eth_hash_filter_info info;
    int ret;


    /* set_hash_global_config 0/1 simple_xor ipv4-udp enable */
    memset(&info, 0, sizeof(info));
    info.info_type = RTE_ETH_HASH_FILTER_GLOBAL_CONFIG;
    info.info.global_conf.hash_func =
                RTE_ETH_HASH_FUNCTION_SIMPLE_XOR;
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                        RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s indev call function rte_eth_dev_filter_ctrl failed\n", __func__);
        return ret;
    }
    ...
    info.info.global_conf.hash_func =
                RTE_ETH_HASH_FUNCTION_SIMPLE_XOR;
    ret = rte_eth_dev_filter_ctrl(outdev->id, RTE_ETH_FILTER_HASH,
                        RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s outdev call function rte_eth_dev_filter_ctrl failed\n", __func__);
        return ret;
    }

    /*
     * Port 1: indev Port 0: outdev
     * set_hash_input_set 1 ipv4-udp none select
     * set_hash_input_set 1 ipv4-udp src-ipv4 add
     * set_hash_input_set 1 ipv4-udp udp-src-port add
     * set_hash_input_set 0 ipv4-udp none select
     * set_hash_input_set 0 ipv4-udp dst-ipv4 add
     * set_hash_input_set 0 ipv4-udp udp-dst-port add 
     */
    ...
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 udp indev clean failed\n", __func__);
        return ret;
    }
    ...
    ret = rte_eth_dev_filter_ctrl(outdev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 udp outdev clean failed\n", __func__);
        return ret;
    }
    ...
    info.info.input_set_conf.field[1] = RTE_ETH_INPUT_SET_L4_UDP_SRC_PORT;
    info.info.input_set_conf.inset_size = 2;
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 udp indev set failed\n", __func__);
        return ret;
    }
    ...
    info.info.input_set_conf.field[1] = RTE_ETH_INPUT_SET_L4_UDP_DST_PORT;
    info.info.input_set_conf.inset_size = 2;
    ret = rte_eth_dev_filter_ctrl(outdev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 udp outdev set failed\n", __func__);
        return ret;
    }
    ...
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 tcp indev clean failed\n", __func__);
        return ret;
    }
    ...
    ret = rte_eth_dev_filter_ctrl(outdev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 tcp outdev clean failed\n", __func__);
        return ret;
    }
    ...
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 tcp indev set failed\n", __func__);
        return ret;
    }
    ...
    ret = rte_eth_dev_filter_ctrl(outdev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    if (ret) {
        RTE_LOG(ERR, NETIF, "%s ipv4 tcp outdev set failed\n", __func__);
        return ret;
    }
    ...
    info.info_type = RTE_ETH_HASH_FILTER_INPUT_SET_SELECT;
    info.info.input_set_conf.op = RTE_ETH_INPUT_SET_SELECT;
    info.info.input_set_conf.flow_type = RTE_ETH_FLOW_NONFRAG_IPV6_UDP;
    info.info.input_set_conf.field[0] = RTE_ETH_INPUT_SET_NONE;
    info.info.input_set_conf.inset_size = 1;
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    ...
    info.info.input_set_conf.op = RTE_ETH_INPUT_SET_ADD;
    info.info.input_set_conf.flow_type = RTE_ETH_FLOW_NONFRAG_IPV6_UDP;
    info.info.input_set_conf.field[0] = RTE_ETH_INPUT_SET_L3_SRC_IP6;
    info.info.input_set_conf.field[1] = RTE_ETH_INPUT_SET_L4_UDP_SRC_PORT;
    info.info.input_set_conf.inset_size = 2;
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    ...
    info.info.input_set_conf.op = RTE_ETH_INPUT_SET_ADD;
    info.info.input_set_conf.flow_type = RTE_ETH_FLOW_NONFRAG_IPV6_UDP;
    info.info.input_set_conf.field[0] = RTE_ETH_INPUT_SET_L3_DST_IP6;
    info.info.input_set_conf.field[1] = RTE_ETH_INPUT_SET_L4_UDP_DST_PORT;
    info.info.input_set_conf.inset_size = 2;
    ret = rte_eth_dev_filter_ctrl(outdev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
    ...
    info.info_type = RTE_ETH_HASH_FILTER_INPUT_SET_SELECT;
    info.info.input_set_conf.op = RTE_ETH_INPUT_SET_SELECT;
    info.info.input_set_conf.flow_type = RTE_ETH_FLOW_NONFRAG_IPV6_TCP;
    info.info.input_set_conf.field[0] = RTE_ETH_INPUT_SET_NONE;
    info.info.input_set_conf.inset_size = 1;
    ret = rte_eth_dev_filter_ctrl(indev->id, RTE_ETH_FILTER_HASH,
                RTE_ETH_FILTER_SET, &info);
...
int netif_set_nat_rss_hash(struct netif_port *indev, struct netif_port *outdev)
{
    if (indev->nic_type != outdev->nic_type) {
        RTE_LOG(ERR, NETIF, "%s Not support diff NIC type\n", __func__);
        return EDPVS_INVAL;
    }

    if (indev->nic_type == NIC_DRV_I40E)
        return netif_i40e_set_nat_rss_hash(indev, outdev);
...
static inline void setup_dev_of_flags(struct netif_port *port)
{
    ...
    if ((port->nic_type == NIC_DRV_I40E) || (port->nic_type == NIC_DRV_ICE))
        port->flag |= NETIF_PORT_FLAG_SUPPORT_FDIR_L3_IP;
    else
        port->flag |= NETIF_PORT_FLAG_SUPPORT_FDIR_L4_PORTMASK;
        ...
        int ntxq, const struct rte_eth_conf *conf)
{
    ...
    if (strncmp(port->dev_info.driver_name, "net_ixgbe", strlen("net_ixgbe")) == 0) {
        port->nic_type = NIC_DRV_IXGBE;
    } else if (strncmp(port->dev_info.driver_name, "net_i40e", strlen("net_i40e")) == 0) {
        port->nic_type = NIC_DRV_I40E;
    } else if (strncmp(port->dev_info.driver_name, "net_ice", strlen("net_ice")) == 0) {
        port->nic_type = NIC_DRV_ICE;
    } else if (strncmp(port->dev_info.driver_name, "net_bond", strlen("net_bond")) == 0) {
        port->nic_type = NIC_DRV_BOND;
    } else {
        port->nic_type = NIC_DRV_UNKNOWN;
    }
...
int netif_flow_create(struct netif_port *port,
         const struct rte_flow_attr *attr,
		 const struct rte_flow_item *patterns,
		 const struct rte_flow_action *actions)
{
    ...
    if (!port->netif_ops->op_flow_create)
      return EDPVS_NOTSUPP;

    rte_spinlock_lock(&g_flow_lock);
    err = port->netif_ops->op_flow_create(port, attr, patterns, actions);
    rte_spinlock_unlock(&g_flow_lock);

    return err;
}

int netif_flow_destroy(struct netif_port *port, int flow_id)
{
    int err;
    assert(port && port->netif_ops);

    if (!port->netif_ops->op_flow_destroy)
     return EDPVS_NOTSUPP;

    rte_spinlock_lock(&g_flow_lock);
    err =  port->netif_ops->op_flow_destroy(port, flow_id);
    rte_spinlock_unlock(&g_flow_lock);
...
// FIX ME, this macro copy for dpdk19.11 header file i40e_ethdev.h
...
static int netif_nic_rss_ip(struct netif_port *port)
{
    if (port->nic_type == NIC_DRV_I40E) {
        return ETH_RSS_IP & I40E_RSS_OFFLOAD_ALL;
    } else if (port->nic_type == NIC_DRV_ICE) {
        return ETH_RSS_IP & ICE_RSS_OFFLOAD_ALL;
    } else {
        return ETH_RSS_IP;
    }
}

static int netif_nic_rss_udp(struct netif_port *port)
{
    if (port->nic_type == NIC_DRV_I40E) {
        return ETH_RSS_UDP & I40E_RSS_OFFLOAD_ALL;
    } else if (port->nic_type == NIC_DRV_ICE) {
        return ETH_RSS_UDP & ICE_RSS_OFFLOAD_ALL;
    } else {
        return ETH_RSS_UDP;
    }
}

static int netif_nic_rss_tcp(struct netif_port *port)
{
    if (port->nic_type == NIC_DRV_I40E) {
        return ETH_RSS_TCP & I40E_RSS_OFFLOAD_ALL;
    } else if (port->nic_type == NIC_DRV_ICE) {
        return ETH_RSS_TCP & ICE_RSS_OFFLOAD_ALL;
    } else {
        return ETH_RSS_TCP;
    }
}
...
static int rss_resolve_proc(struct netif_port *port, char *rss)
{
    int rss_value = 0;

    if (!strcmp(rss, "all"))
        rss_value = netif_nic_rss_ip(port) | netif_nic_rss_tcp(port) | netif_nic_rss_udp(port);
    else if (!strcmp(rss, "ip"))
        rss_value = netif_nic_rss_ip(port);
    else if (!strcmp(rss, "tcp"))
        rss_value = netif_nic_rss_tcp(port);
    else if (!strcmp(rss, "udp"))
        rss_value = netif_nic_rss_udp(port);
    ...
    if (cfg_stream) {
        ...
        for (index = 0; index < strlen(cfg_stream->rss); index++) {
            if (cfg_stream->rss[index] == ' ') {
                ...
                rss[rss_index] = '\0';
                rss_index = 0;
                port->dev_conf.rx_adv_conf.rss_conf.rss_hf |= rss_resolve_proc(port, rss);
...
static int fdir_filter_flush(const struct netif_port *port)
{
    ...
    /* TODO: FIX ME, zhutao disable follow line */
    //if (rte_eth_dev_filter_ctrl(port->id, RTE_ETH_FILTER_FDIR,
...
int netif_port_start(struct netif_port *port)
{
    ...
    // device configure check NIC type first: ixgbe support
    if (port->flag & NETIF_PORT_FLAG_SUPPORT_FDIR_L4_PORTMASK)
        if ((ret = netif_port_fdir_dstport_mask_set(port)) != EDPVS_OK)
            return ret;

    rte_memcpy(&dev_conf, &port->dev_conf, sizeof(struct rte_eth_conf));
    if (port->type == PORT_TYPE_BOND_MASTER) {
        dev_conf.rxmode.offloads = 0;
        dev_conf.txmode.offloads = 0;
    }
    ret = rte_eth_dev_configure(port->id, port->nrxq, port->ntxq, &dev_conf);
    if (ret < 0 ) {
    ...
    if (port->nrxq > 0) {
        ...
        for (qid = 0; qid < port->nrxq; qid++) {
            ret = rte_eth_rx_queue_setup(port->id, qid, port->rxq_desc_nb,
                    port->socket, &rxq_conf, pktmbuf_pool[port->socket]);
            if (ret < 0) {
    ...
    if (port->ntxq > 0) {
        for (qid = 0; qid < port->ntxq; qid++) {
            memcpy(&txq_conf, &port->dev_info.default_txconf, sizeof(struct rte_eth_txconf));
            if (port->type == PORT_TYPE_BOND_MASTER) {
                txq_conf.offloads = 0;
            }
            ret = rte_eth_tx_queue_setup(port->id, qid, port->txq_desc_nb,
                    port->socket, &txq_conf);
            if (ret < 0) {
    ...
    if ((port->nic_type == NIC_DRV_I40E) || (port->nic_type == NIC_DRV_ICE)) {
        ret = rte_eth_allmulticast_enable(port->id);
        if (ret != 0) {
            RTE_LOG(WARNING, NETIF, "%s: enable allmulticast failed, err:%d.\n", __func__, ret);
            return ret;
        }
        RTE_LOG(WARNING, NETIF, "%s: Enable allmulticast NIC.\n", __func__);
    }
...
/* TODO: FIX ME, some member not declear in dpdk1911 */
...
static inline int do_lcore_job(struct netif_lcore_loop_job *job, int high_stat)
{
    int ret;
#if defined(CONFIG_RECORD_BIG_LOOP) || defined(CONFIG_RECORD_LOOP_CYCLE)
    uint64_t job_start, job_end;
    job_start = rte_get_timer_cycles();
#endif

    ret = job->func(job->data, high_stat);
    ...
    struct job_cycle_record *records = &job->records[rte_lcore_id()];

    job_end = rte_get_timer_cycles();
    if (!((job_end - job_start) >> 8))
        return ret;

    records->index++;
    if (unlikely(records->index == JOB_CYCLE_RECORD_COUNT))
        records->index = 0;

    records->cycle[records->index] = (job_end - job_start) >> 8; /* prevent overflow */
    if (unlikely(records->cycle[records->index] < records->min_cycle)) {
        records->min_cycle = records->cycle[records->index];
    } else if (unlikely(records->cycle[records->index] > records->max_cycle)) {
        records->max_cycle = records->cycle[records->index];
    }
#endif
...
#ifdef CONFIG_RECORD_LOOP_CYCLE
bool g_netif_cycle_dump[DPVS_MAX_LCORE];
uint32_t g_netif_cycle_precision[DPVS_MAX_LCORE];   /* Ignore the low bits */
uint8_t g_netif_cycle_clean[DPVS_MAX_LCORE];   /* Ignore the low bits */


static void netif_lcore_dump_job_cycle(struct netif_lcore_loop_job *job)
{
    struct job_cycle_record *records = &job->records[rte_lcore_id()];
    uint32_t mid_cycle = 0, i, n = 0;
    char buffer[1024] = "";

    for (i=0; i<JOB_CYCLE_RECORD_COUNT; i++) {
        n += snprintf(buffer+n, 1023-n, "%d  ", records->cycle[i]);
        mid_cycle += records->cycle[i];
    }

    mid_cycle = mid_cycle / JOB_CYCLE_RECORD_COUNT;
    RTE_LOG(INFO, NETIF, "lcore[%d] job[%s] min: %d, max: %d, mid: %d\n\tSample cycle: %s\n",
                rte_lcore_id(), job->name,
                records->min_cycle >> g_netif_cycle_precision[rte_lcore_id()],
                records->max_cycle >> g_netif_cycle_precision[rte_lcore_id()],
                mid_cycle >> g_netif_cycle_precision[rte_lcore_id()], buffer);

    if (g_netif_cycle_clean[rte_lcore_id()])
        memset(records, 0, sizeof(*records));
}
#endif

/* 
 * The function increase the credit in steady steps.
 * If packets transmitted increase in faster steps, the credits increased can
 * not make is_lcore_idle() return ture.
 */
static inline void inc_lcore_tx_idle_credit(lcoreid_t cid)
{
    RTE_PER_LCORE(this_lcore_tx_idle_credit)++;
    RTE_PER_LCORE(this_lcore_prev_opackets) = lcore_stats[cid].opackets + 
                                              RTE_PER_LCORE(this_lcore_tx_idle_credit);
}

static inline void rec_lcore_tx_idle_credit(lcoreid_t cid)
{
    RTE_PER_LCORE(this_lcore_tx_idle_credit) = 0;
    RTE_PER_LCORE(this_lcore_prev_opackets) = lcore_stats[cid].opackets;
}

static bool is_lcore_idle(lcoreid_t cid)
{
    if (lcore_stats[cid].opackets <= RTE_PER_LCORE(this_lcore_prev_opackets) ||
        lcore_stats[cid].opackets - RTE_PER_LCORE(this_lcore_prev_opackets) <= RTE_PER_LCORE(this_lcore_tx_idle_threshold)) {
        return true;
    } else {
        return false;
    }
...
#endif
#ifdef CONFIG_RECORD_LOOP_CYCLE
        netif_cycle_dump = g_netif_cycle_dump[rte_lcore_id()];
#endif
        lcore_stats[cid].lcore_loop++;
...
#ifdef CONFIG_RECORD_LOOP_CYCLE
                if (unlikely(netif_cycle_dump))
                    netif_lcore_dump_job_cycle(job);
#endif
            }
...
#ifdef CONFIG_RECORD_LOOP_CYCLE
        if (netif_cycle_dump) {
            g_netif_cycle_dump[rte_lcore_id()] = false;
            g_netif_cycle_clean[rte_lcore_id()] = 0;
        }
#endif
    }
    return EDPVS_OK;
...
{
    /* TODO: FIX ME, if you want to use pci device later. */
#if 0
    if (dev_info->pci_dev)
