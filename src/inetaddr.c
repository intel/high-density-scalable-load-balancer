 ...
 *
 * Changes:
 *    Zhu Tao: In order to support 100m session, the LADDR of fnat is derived.
 */
#include <stdlib.h>
#include <stdbool.h>
#include <assert.h>
...
struct netif_port *
inet_lookup_dev_by_addr(int af, const union inet_addr *addr)
{
    struct netif_port *dev = NULL;
    struct inet_device *idev = NULL;
    portid_t pid;

    for (pid=0; pid<netif_port_count(); pid++) {
        ...
        if (dev) {
            ...
            if (__ifa_lookup(idev, addr, 0, af)) {
                idev_put(idev);
                return dev;
            }
...
static int inet_addr_del_multiply_one(lcoreid_t cid, int af, union inet_addr *laddr, void *arg, int index)
{
    int err;
    struct inet_addr_multiply_arg *marg = arg;
    char addr_str[64];

    err = __ifa_remove(marg->idev, laddr, marg->plen, &marg->ifa, af);
    if (err == EDPVS_OK) {
        dpvs_timer_cancel(&marg->ifa->timer, true);
        ifa_add_del_mcast(marg->ifa, false);
        ifa_del_route(marg->ifa);
        idev_put(marg->idev);
        rte_free(marg->ifa);
        rte_atomic32_dec(&marg->idev->ifa_cnt);
        rte_atomic32_dec(&in_addr_cnt);
        inet_ntop(af, &laddr->in.s_addr, addr_str, sizeof(addr_str));
...
{
    struct inet_ifaddr *ifa, *ent;
    struct inet_device *idev;
    int err = 0;
    char addr_str[64];
    struct inet_addr_multiply_arg arg;
    ...
    rte_rwlock_write_lock(&in_addr_lock);
    if ((ent = __ifa_lookup(idev, addr, plen, af)) != NULL) {
        if (ent->flags & IFA_F_SAPOOL_MULTIPLY) {
            arg.ifa = ent;
            arg.idev = idev;
            arg.plen = plen;
            laddr_multiply_process(af, (union inet_addr *)addr, &inet_addr_del_multiply_one, NULL, &arg);
        } else {
            ifa = ent;
...
static int __ifa_add_set(int af, const struct netif_port *dev,
                       const union inet_addr *addr, uint8_t plen,
                       const union inet_addr *bcast,
                       uint32_t valid_lft, uint32_t prefered_lft,
                       uint8_t scope, uint32_t flags, bool create,
                       struct inet_ifaddr *ifa, uint32_t lcore_id)
{
    struct inet_device *idev = NULL;
    ...
    if (!ifa) {
        ...
        ifa->af   = af;
        ifa->idev = idev;
        memcpy(&ifa->addr, addr, sizeof(*addr));
        ifa->plen = plen;
        ifa->flags = flags;
        ifa->lcore_id = lcore_id;
...
static int inet_addr_add_multiply_one_rollback(lcoreid_t cid, int af, union inet_addr *laddr, void *arg, int index)
{
    struct inet_addr_multiply_arg *marg = arg;

    return inet_addr_del(af, (struct netif_port *)(marg->dev), laddr, marg->plen);
}

static int inet_addr_add_multiply_one(lcoreid_t cid, int af, union inet_addr *laddr, void *arg, int index)
{
    struct inet_addr_multiply_arg *marg = arg;

    return __ifa_add_set(af, marg->dev, laddr, marg->plen, marg->bcast, marg->valid_lft, marg->prefered_lft,
                               marg->scope, marg->flags, true, marg->ifa, cid);
}
                       ...
                       uint8_t scope, uint32_t flags, bool create)
{
    ...
    if (!(flags & IFA_F_SAPOOL_MULTIPLY))
        return __ifa_add_set(af, dev, addr, plen, bcast, valid_lft, prefered_lft,
                               scope, flags, true, ifa, 0);

    arg.dev = dev;
    arg.plen = plen;
    arg.bcast = bcast;
    arg.valid_lft = valid_lft;
    arg.prefered_lft = prefered_lft;
    arg.scope = scope;
    arg.flags = flags;
    arg.ifa = ifa;

    return laddr_multiply_process(af, (union inet_addr *)addr, inet_addr_add_multiply_one,
                        inet_addr_add_multiply_one_rollback, &arg);
}
