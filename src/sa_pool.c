 ...
 *
 * Changes:
 *      Zhu Tao: Support dpdk generic flow API.
 */
#include <stdint.h>
#include <assert.h>
#include <arpa/inet.h>
...
/* Now support udp */
static int sa_add_filter_v3(int af, struct netif_port *dev, lcoreid_t cid,
              uint8_t proto, const union inet_addr *addr)
{
    ...
    if (proto == IPPROTO_TCP) {
        ...
        tcp.hdr.dst_port = sa_fdirs[cid].port_base;
        tcp_mask.hdr.dst_port = ntohs(sa_fdirs[cid].mask);
        ...
        udp.hdr.dst_port = sa_fdirs[cid].port_base;
        udp_mask.hdr.dst_port = ntohs(sa_fdirs[cid].mask);
...
static inline int sa_add_filter(int af, struct netif_port *dev, lcoreid_t cid,
                                const union inet_addr *dip, __be16 dport,
                                uint32_t filter_id[MAX_FDIR_PROTO], uint8_t proto)
{
    return sa_add_filter_v3(af, dev, cid, proto, dip);
}
...
int sa_pool_create(struct inet_ifaddr *ifa, uint16_t low, uint16_t high)
{
    ...
    for (cid = 0; cid < RTE_MAX_LCORE; cid++) {
        ...
        flow_id0 = sa_add_filter(ifa->af, ifa->idev->dev, cid, &ifa->addr,
                            fdir->port_base, filtids, IPPROTO_TCP);
        flow_id1 = sa_add_filter(ifa->af, ifa->idev->dev, cid, &ifa->addr,
                            fdir->port_base, filtids, IPPROTO_UDP);
        if (flow_id0 < 0 || flow_id1 < 0) {
            sa_pool_free_hash(ap);
            rte_free(ap);
            err = EDPVS_INVAL;
            goto errout;
        }
        ap->filter_id[0] = flow_id0;
        ap->filter_id[1] = flow_id1;
