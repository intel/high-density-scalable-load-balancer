...
#define DP_VS_SVC_F_LB_CORE         0x0400

#define DP_VS_MAX_SERVER_ID     8
#define DP_VS_MAX_DIRECTION     2
#define DP_VS_FLOW_DIR_OUT      0
#define DP_VS_FLOW_DIR_IN       1
#define DP_VS_FLOW_DIR_SNAT_OUT    2
#define DP_VS_FLOW_DIR_NAT_OUT    3
#define DP_VS_F_SERVER_ID_FREE  0
#define DP_VS_F_SERVER_ID_BUSY  1
#define DP_VS_FLOW_ID_BASE  0x10
#define DP_VS_MARK_FLOW_ID_UNKNOWN -1
...
struct dp_vs_service {
    ...
    /* FNAT only */
    struct dp_vs_fnat_laddr laddrs[RTE_MAX_LCORE];
...
static inline int
dp_vs_server_add_mark_flow(struct dp_vs_service *svc)
{
    struct netif_port *dev = inet_lookup_dev_by_addr(svc->af, &svc->addr);
    int flow_id;
    ...
    flow_markid.value = svc->markid;
    flow_markid.dir = DP_VS_FLOW_DIR_IN;

    flow_id = netif_flow_mark(svc->af, svc->proto, svc->port, flow_markid.value, &svc->addr, dev);
    if (flow_id < 0) {
...
static inline int
dp_vs_server_del_mark_flow(struct dp_vs_service *svc)
{
    struct netif_port *dev = inet_lookup_dev_by_addr(svc->af, &svc->addr);
    ...
    return netif_flow_destroy(dev, svc->mark_flowid);
}
...
#define SOCKOPT_SVC_GET_CMD_MAX  DPVS_SO_GET_LB_DEBUG
