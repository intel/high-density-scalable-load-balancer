...
enum {
    ...
    NETIF_PORT_FLAG_SUPPORT_FDIR_L4_PORTMASK= (0x1<<13),
    NETIF_PORT_FLAG_SUPPORT_FDIR_L3_IP      = (0x1<<14),
};
...
#define NETIF_MAX_FLOW_ID           16 // 2 * 8

enum netif_flow_type {
    NETIF_FLOW_TYPE_UNKNOWN  = 0,
    NETIF_FLOW_TYPE_IPV4_UDP_IN  = 1,
    NETIF_FLOW_TYPE_IPV4_UDP_OUT = 2,
    NETIF_FLOW_TYPE_IPV4_TCP = 3,
    NETIF_FLOW_TYPE_IPV6_UDP = 4,
    NETIF_FLOW_TYPE_IPV6_TCP = 5,
    NETIF_FLOW_TYPE_MAX = 31,
};

typedef void (*netif_flow_handler_t)(struct rte_mbuf *mbuf, uint8_t markid);


/************************* lcore conf  ***************************/
struct rx_partner;

#define NETIF_FLOW_GENERAL  0
#define NETIF_FLOW_INVALID  255
...
struct netif_queue_conf
{
    ...
    uint8_t first_flowid;
    uint8_t marked_cnt;
    uint8_t ofb_data[NETIF_MAX_PKT_BURST];
...
struct netif_lcore_stats
{
    ...
    uint64_t impackets; /* Total number of successfully received packets with fdir mark id. */
...
enum netif_lcore_job_type {
    ...
    NETIF_LCORE_JOB_HIGH      = 1,
    NETIF_LCORE_JOB_LOOP      = 2,
    NETIF_LCORE_JOB_SLOW      = 3,
	NETIF_LCORE_JOB_IDLE      = 4,
    NETIF_LCORE_JOB_TYPE_MAX  = 5,
};

enum netif_principal_status {
    NETIF_PRINCIPAL_STAT_IDLE,
    NETIF_PRINCIPAL_STAT_LOW,
    NETIF_PRINCIPAL_STAT_HIGH,
    NETIF_PRINCIPAL_STAT_FULL,
    NETIF_PRINCIPAL_STAT_MAX,
};

#define JOB_CYCLE_RECORD_COUNT  13
struct job_cycle_record {
    uint32_t max_cycle;
    uint32_t min_cycle;
    uint32_t index;
    uint32_t cycle[JOB_CYCLE_RECORD_COUNT];
}__rte_cache_aligned;

struct netif_lcore_loop_job
{
    char name[32];
    int (*func)(void *arg, int high_stat);
    void *data;
    ...
    struct job_cycle_record records[DPVS_MAX_LCORE];
#endif
...
struct netif_ops {
    ...
    int (*op_flow_create)(struct netif_port *dev,
         const struct rte_flow_attr *attr,
		 const struct rte_flow_item *pattern,
		 const struct rte_flow_action *actions);
    int (*op_flow_destroy)(struct netif_port *dev, int flow_id);
...
#define NETIF_FLOW_MAX      0xFFFFFFFF
struct netif_flow_rule {
    void            *flow;
    uint32_t         flow_id;
    struct list_head list;
};
...
struct netif_port {
    ...
    uint32_t                nic_type;                   /* nic driver type */
