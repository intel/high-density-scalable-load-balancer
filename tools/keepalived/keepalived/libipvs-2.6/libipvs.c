...
int ipvs_update_service_by_options(ipvs_service_t *svc, uint64_t options)
{
...
static void ipvs_fill_ipaddr_conf(ipvs_laddr_t *laddr, struct inet_addr_param *param)
{
	...
	param->flags |= IFA_F_SAPOOL_MULTIPLY;
	return;
}
...
int ipvs_start_daemon(ipvs_daemon_t *dm)
{
	...
	return dpvs_setsockopt(SOCKOPT_SET_STARTDAEMON,
			       dm, sizeof(*dm));
}
...
int ipvs_stop_daemon(ipvs_daemon_t *dm)
{
	...
	return dpvs_setsockopt(SOCKOPT_SET_STOPDAEMON,
			       dm, sizeof(*dm));
}


int ipvs_set_daemon(ipvs_daemon_t *dm)
{
	ipvs_func = ipvs_set_daemon;
	return dpvs_setsockopt(SOCKOPT_SET_SETDAEMON,
			       dm, sizeof(*dm));
}
...
struct lb_sync_daemon_stats *ipvs_get_daemon(void)
{
    int err;
    ipvs_daemon_t *dm;
    size_t len_rcv;
    struct lb_sync_daemon_stats *remote_stats;
    struct lb_sync_daemon_stats *local_stats = NULL;

    dm = calloc(1, sizeof(ipvs_daemon_t));
    if (dm == NULL) {
        fprintf(stderr, "%s: out of memory\n", __func__);
        return NULL;
    }
    ipvs_func = ipvs_get_daemon;
    err = dpvs_getsockopt(SOCKOPT_GET_DAEMON, dm, sizeof(*dm),
                              (void **)&remote_stats, &len_rcv);
    if (err != 0 || !remote_stats) {
        free(dm);
        fprintf(stderr, "%s: got errcode %d from dpvs_getsockopt\n", __func__, err);
        return NULL;
    }

    if (len_rcv != sizeof(*remote_stats) + remote_stats->n * sizeof(struct lb_sync_daemon_stat)) {
        fprintf(stderr, "%s: result length not match\n", __func__);
        goto end;
    }

    local_stats = calloc(1, len_rcv);
    if (NULL == local_stats) {
        fprintf(stderr, "%s: out of memory\n", __func__);
        goto end;
    }

    memcpy(local_stats, remote_stats, len_rcv);

end:
    dpvs_sockopt_msg_free(remote_stats);
    free(dm);
    return local_stats;
}
...
const char *ipvs_strerror(int err)
{
	...
	struct table_struct {
		...
		{ ipvs_add_blklst, EEXIST, "blocklist address already exists" },
