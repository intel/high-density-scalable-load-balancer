...
/* Check if a blocklist address entry is in list */
static int
...
clear_diff_blklst_entry(list old, list new, virtual_server_t * old_vs)
{
        ...
        for (e = LIST_HEAD(old); e; ELEMENT_NEXT(e)) {
                ...
                if (!blklst_entry_exist(blklst_entry, new)) {
                        log_message(LOG_INFO, "VS [%s-%d] in blocklist address group %s no longer exist\n"
...
/* Clear the diff blocklist address of the old vs */
static int
clear_diff_blklst(virtual_server_t * old_vs)
{
        ...
        /*
         *  If old vs  didn't own blocklist address group, 
         * then do nothing and return 
         */
        if (!old_vs->blklst_addr_gname)
                return 1;

        /* Fetch blocklist address group */
	...
	for (e = LIST_HEAD(l); e; ELEMENT_NEXT(e)) {
		...
		if (!vs_exist(vs, &empty_vsg)) {
			...
			if (!clear_diff_laddr(vs))
				return 0;
                        /* perform blocklist address diff */
